...
describe('processPatchVinrequest', () => {
    it('should format the objects correctly', () => {
        ...
        const {newRecord, recordToArchive} =  processPatchVinRequest(carPostData, event as unknown as APIGatewayProxyEvent)

        expect(newRecord).not.toEqual(recordToArchive)
        expect(newRecord.vin).toBe('NEWVIN')
        expect(newRecord.techRecord_createdById).toBe('123123')
        expect(newRecord.techRecord_createdByName).toBe('John Doe')
        expect(recordToArchive.vin).toBe('AA11100851')
        expect(recordToArchive.techRecord_updatedById).toBe('123123')
        expect(recordToArchive.techRecord_updatedByName).toBe('John Doe')
    })
})